...
struct _E820Map {
  ...
  struct {
    ...
    uint64_t size;
    uint32_t type;
  } __packed map[E820MAX];
};
typedef struct _E820Map E820Map;
...
struct _Page {
  ...
  uint32_t flags;
  // used in buddy system, stores the order (the X in 2^X) 
  // of the continuous memory block
  unsigned int property;
  // used in buddy system, the No. of zone which the page belongs to
  int zone;
  // free list link
  KListEntry page_link;
  // stores a swapped-out page identifier
  swap_entry_t swap_index;
  //KListEntry swap_link; // swap hash link
};
...
#define PG_extraslab                3 // page frame is included in a extra slab
#define PG_dirty                    4 // the page has been modified
#define PG_swap                     5 // the page is in the active or inactive page list (and swap hash table)
#define PG_active                   6 // the page is in the active page list

#define PageSetFlag(PAGE, FLAG)     kset_bit(PG_##FLAG, &((PAGE)->flags))
#define PageHasFlag(PAGE, FLAG)     ktest_bit(PG_##FLAG, &((PAGE)->flags))
#define PageClearFlag(PAGE, FLAG)   kclear_bit(PG_##FLAG, &((PAGE)->flags))
